...
# Use Llama-2-7B instead of Llama-3.1-8B (no approval required)
...
# We use the weights from the 20th layer (index 19) to initialize the lightweight layer
print("Initializing lightweight layer with layer 19 weights...")
model_dict['replace_layer.self_attn.q_proj.weight'] = llama_dict['model.layers.19.self_attn.q_proj.weight']
model_dict['replace_layer.self_attn.k_proj.weight'] = llama_dict['model.layers.19.self_attn.k_proj.weight']
model_dict['replace_layer.self_attn.v_proj.weight'] = llama_dict['model.layers.19.self_attn.v_proj.weight']
model_dict['replace_layer.self_attn.o_proj.weight'] = llama_dict['model.layers.19.self_attn.o_proj.weight']
model_dict['replace_layer.mlp.gate_proj.weight'] = llama_dict['model.layers.19.mlp.gate_proj.weight']
model_dict['replace_layer.mlp.up_proj.weight'] = llama_dict['model.layers.19.mlp.up_proj.weight']
model_dict['replace_layer.mlp.down_proj.weight'] = llama_dict['model.layers.19.mlp.down_proj.weight']
model_dict['replace_layer.input_layernorm.weight'] = llama_dict['model.layers.19.input_layernorm.weight']
model_dict['replace_layer.post_attention_layernorm.weight'] = llama_dict['model.layers.19.post_attention_layernorm.weight']

model.load_state_dict(model_dict)
del llama_model

print("Setting up trainable parameters...")
# Freeze all parameters except replace_layer
...
for epoch in range(1):
    ...
    for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader), desc="Training"):
        with accelerator.accumulate(model):
            ...
            outputs = outputs.last_hidden_state[1]   # Lightweight layer output
        ...
        if (step + 1) % 500 == 0:            
            ...
            if eval_loss < best_loss:
                ...
                if accelerator.is_main_process:
                    unwrapped_model.save_pretrained("./best_llama2_pruned_model")
                    tokenizer.save_pretrained("./best_llama2_pruned_model")
                    print("Model saved to ./best_llama2_pruned_model")
...
print("Final model saved to ./best_llama2_pruned_model") 